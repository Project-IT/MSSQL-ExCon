...
public class EventDeleter {
    ...
    public void delete(String user, Connection myConn) throws SQLException {
        ...
        PreparedStatement ps = myConn.prepareStatement("SELECT OutlookUID FROM [confluence].[dbo].[OutlookUIDtable] WHERE Username='" + user+"'");
        ...
        if (tableIDs.size() != 0) {
            ...
            for (Object ID : tableIDs) {
               ...
               // PreparedStatement ps2 = myConn.prepareStatement("SELECT ConfluenceUID FROM confluence.outlookuidtable WHERE OutlookUID='" + ID + "'");
                PreparedStatement ps2 = myConn.prepareStatement("SELECT ConfluenceUID FROM [confluence].[dbo].[OutlookUIDtable] WHERE OutlookUID='" + ID + "'");
                ...
                if (rs2.next()) {
                    ...
                    String calendarDel = "DELETE FROM [confluence].[dbo].[AO_950DC3_TC_EVENTS] WHERE VEVENT_UID='" + rs2.getString("ConfluenceUID") + "'";
                   ...
                   // String sqlDel = "DELETE FROM confluence.outlookuidtable WHERE OutlookUID='" + ID + "'";
                    String sqlDel = "DELETE FROM [confluence].[dbo].[OutlookUIDtable] WHERE OutlookUID='" + ID + "'";
